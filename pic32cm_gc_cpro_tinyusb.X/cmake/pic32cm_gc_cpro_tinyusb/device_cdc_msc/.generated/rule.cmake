...
function(pic32cm_gc_cpro_tinyusb_device_cdc_msc_device_cdc_msc_XC32_assemble_rule target)
    set(options
        "-g"
        "${ASSEMBLER_PRE}"
...
function(pic32cm_gc_cpro_tinyusb_device_cdc_msc_device_cdc_msc_XC32_assembleWithPreprocess_rule target)
    set(options
        "-x"
        "assembler-with-cpp"
        "-g"
    ...
    target_compile_definitions(${target}
        PRIVATE "__DEBUG=1"
        PRIVATE "XPRJ_device_cdc_msc=device_cdc_msc")
    target_include_directories(${target} PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}/../../..")
endfunction()
function(pic32cm_gc_cpro_tinyusb_device_cdc_msc_device_cdc_msc_XC32_compile_rule target)
    set(options
        "-g"
        "${CC_PRE}"
        "-x"
    ...
    target_include_directories(${target}
        ...
        PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}/../../../../src/examples/device/cdc_msc"
...
function(pic32cm_gc_cpro_tinyusb_device_cdc_msc_device_cdc_msc_XC32_compile_cpp_rule target)
    set(options
        "-g"
        "${CC_PRE}"
        "${DEBUGGER_NAME_AS_MACRO}"
...
function(pic32cm_gc_cpro_tinyusb_device_cdc_msc_link_rule target)
    set(options
        "-g"
        "${MP_EXTRA_LD_PRE}"
        ...
        "-Wl,--defsym=__MPLAB_BUILD=1${MP_EXTRA_LD_POST},--script=${pic32cm_gc_cpro_tinyusb_device_cdc_msc_LINKER_SCRIPT},--defsym=__MPLAB_DEBUG=1,--defsym=__DEBUG=1,--defsym=_min_heap_size=512,--gc-sections,-L${CMAKE_CURRENT_SOURCE_DIR}/../../..,--memorysummary,memoryfile.xml"
        "-mdfp=${PACK_REPO_PATH}/Microchip/PIC32CM-GC00_DFP/1.0.27")
    list(REMOVE_ITEM options "")
    target_link_options(${target} PRIVATE "${options}")
    target_compile_definitions(${target} PRIVATE "XPRJ_device_cdc_msc=device_cdc_msc")
endfunction()
